...
async function getTopList(ctx, next) {
    ...
    await createWebAPIRequest('music.163.com', action, 'POST', data, cookie).then(res => {
...
async function getMusicDetail(ctx, next) {
    ...
    await createWebAPIRequest('music.163.com', '/weapi/v3/song/detail', 'POST', data, cookie).then(
        res => {
            ctx.response.body = res
        }
    )
...
async function search(ctx, next) {
    ...
    await createWebAPIRequest('music.163.com', '/weapi/search/get', 'POST', data, cookie).then(
        res => {
            ctx.response.body = res
        }
    )
...
async function getTopArtists(ctx, next) {
    ...
    await createWebAPIRequest('music.163.com', `/weapi/artist/top`, 'POST', data, cookie).then(
        res => {
            ctx.response.body = res
        }
    )
...
async function getBanner(ctx, next) {
    ...
    await getData(options).then(res => {
        ...
        const banners = pattern.exec(res)[1]
        var res = JSON.stringify(eval(`({code:200,banners:${banners}})`))
        ctx.body = res
    })
}
