...
function download(page_no, file_path) {
    ...
    try {
        ...
        request.get(url, { headers }).pipe(fs.createWriteStream(file_name))
...
function main() {
    ...
    request.get(mziTu, options, (err, resp) => {
        if (err) {
            return
        }
        ...
        console.log($('div.nav-links').html())
    })
